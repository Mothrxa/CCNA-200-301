...
- *Router as DHCP Server*
	- (config)#ip dhcp excluded-address **first-ip-address last-ip-address** (exclude a range of address from dhcp pool)
	- (config)#ip dhcp pool **pool-name** (creates a dhcp pool and enters dhcp config mode)
		- (dhcp-config)#network **network-id** [**/network-mask**]
		- (dhcp-config)#dns-server **dns-address**
		- (dhcp-config)#domain-name **name**
		- (dhcp-config)#default-router **router-gateway**
		- (dhcp-config)#lease {**days hours minutes** | infinite}
- *Router as DHCP Relay Agent*
	- (config-if)#ip helper-address **server-ip**
- *Router as DHCP Client* 
	- (config-if)#ip address dhcp 
- *Router as DNS Server*
	- (config)#ip dns server
	- (config)#ip host **device-name device-ip** (hostname/ip mapping)
	- (config)#ip name-server **server-ip** (external dns server)
	- (config)#ip domain lookup (enabled by default on the router to perform dns queries)
	- (config)#ip domain name **fqdn** (default domain name)
- *Show DHCP Config*
...
- *Show DNS Config*
	- (Router)#show hosts
- *PC DHCP Config*
	- >ipconfig /release (sends DHCP Release Message)
	- >ipconfig /renew (sends DHCP Discover Message)
- *PC Commands*
	- >ipconfig /all
	- >ipconfig /displaydns (displays local DNS cache)
